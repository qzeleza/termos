...
build    := ziva.NewFuncTask("Сборка", assembleCluster).WithoutSuccessLabel()
...
- **Убирайте метку завершения, если она не нужна.** Для `FuncTask` используйте `WithoutSuccessLabel()` или опцию `WithoutSuccessLabelOption()`, чтобы скрыть подпись «ГОТОВО».
